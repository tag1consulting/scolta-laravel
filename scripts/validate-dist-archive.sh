...
# config, tooling, the committed composer.lock) out of what every Composer
# consumer downloads, and an over-broad line silently DROPS a runtime file and
...
# tarball; this is the scolta-laravel port.
#
...
SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/scolta-laravel-dist.XXXXXX")"
...
# Size cap. Measured clean archive (git archive HEAD, 2026-06-14) = 358,400
# bytes (~0.35 MB; pure PHP/Blade, no vendored binaries). Cap set at roughly 2x
# to catch accidental bloat (a stray index, a build artifact, a committed
# vendor/ tree) while tolerating normal growth. Bump deliberately with a fresh
# measurement if the package grows.
MAX_BYTES=720000
